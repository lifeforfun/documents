...
##### WebSocket挥手
> 一旦发送或接收到挥手帧,WebSocket连接将处于CLOSING状态.
> 当下层的TCP连接断开时,WebSocket连接处于CLOSED状态.
> WebSocket挥手完成且下层TCP正常断开时,WebSocket是清洁断开的.
> 当由于WebSocket无法建立连接时,WebSocket是非清洁断开的.
> 如果挥手帧不包含状态码,WebSocket断开连接码默认是1005.
> 如果断开连接时未收到挥手帧,WebSocket断开连接码是1006.
> 两端的WebSocket断开连接码可能不一致,尤其是在两方同时发送挥手帧时.
> 挥手帧可携带一个状态码以及其后的UTF-8编码数据,数据解释不在协议内定义.
> 挥手帧可不包含状态码和或原因字符串.

> 首次重连应在一个随即时间的延迟后开始,推荐0-5秒,之后每次失败都会在稍微更长的延迟后重试.
> 延迟时间递增算法可使用截断二进制指数退避算法(truncated binary exponential backoff).
...
- 1003: 数据类型不被识别
- 1004: 保留
- 1005: 保留, 不能用于挥手帧中. 用于在期望获得状态码的应用指明没有状态码.
- 1006: 保留, 不能用于挥手帧中. 用于在期望获得状态码的应用指明连接异常断开并且未发送挥手帧.
- 1007: 指明消息内容和其指定的消息类型不一致(例如,在文本消息中存在非UTF-8数据)
- 1008: 接收到的消息违背方针政策而断开连接. 在没有更适合的状态码时可使用次状态码
- 1009: 数据过大
- 1010: (客户端)期望与服务端协定一个或多个扩展,但服务端在WebSocket握手时未返回
- 1011: 服务端由于未满足期望的条件而断开连接
- 1015: 保留, 不能用于挥手帧中. 用于在期望获得状态码的应用指明TLS握手失败.
